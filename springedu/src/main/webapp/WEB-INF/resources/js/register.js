...
function checkValid(){
  ...
  //2) 비밀번호 체크
    if(!isPassword(pwEle.value)) {
      pw_errmsg.innerHTML = "비밀번호가 잘못되었습니다.(숫자,영문 포함 8~10자리)";
      flag = flag && false;    
    }else{
      pw_errmsg.innerHTML = "";
    }
  //3) 비밀번호 재확인
    ...
    if(!isPassword(pwChkEle.value)) {
      pwChk_errmsg.innerHTML = "비밀번호가 잘못되었습니다.(숫자,영문 포함 8~10자리)";
      flag = flag && false;
    }else{
  ...
  //7) 유효성 통과하면 메인화면으로 이동
    if(flag) {
        alert('SI BEL HOMME 회원이 된것을 진심으로 축하드립니다!');
        alert('이제 로그인 화면으로 이동합니다. 로그인 후 이용해 주세요.');
        location.href="sibelhomme-login.html";
    }
